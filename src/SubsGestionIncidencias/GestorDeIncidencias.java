			...
			if (DNI.length() == 9) {
				...
				if (dniCount == 8) {
					...
					if (!Character.isAlphabetic(DNI.charAt(DNI.length() - 1))) {
						System.out.println("el dni debe contener 8 núemros seguidos de una letra");
						return null;
					}
			...
			if (ID.length() > 0 && ID.length() <= 20) {
				...
				for (Incidencia i : incidencias) {
					...
					if (i.getID().equals(ID)) {
						...
						if (campo == null) {
							// aviso al usuario
							System.out.println("El campo especificado es null, no se peude realizar esta acción");
						} else {
